...
class PlutonianEris(Eris):
    ...
    def __init__(self, lines):
        ...
        self.outer_transitions = defaultdict(set)
        for x in self.RIGHT:
            self.outer_transitions[x].add(add(self.CENTRE, Directions.RIGHT.value))
        for x in self.LEFT:
            self.outer_transitions[x].add(add(self.CENTRE, Directions.LEFT.value))
        for x in self.TOP:
            self.outer_transitions[x].add(add(self.CENTRE, Directions.UP.value))
        for x in self.BOTTOM:
            self.outer_transitions[x].add(add(self.CENTRE, Directions.DOWN.value))
    ...
    def step(self):
        ...
        new_layers = {}
        for depth, bugs in self.layers.items():
            new_bugs = set()
            for y in range(self.HEIGHT):
                for x in range(self.WIDTH):
                    current = (x, y)
                    if current == self.CENTRE:
                        continue
                    count = 0
                    for depth_change, (p, q) in self.neighbours(current):
                        if (
                            depth + depth_change in self.layers
                            and (p, q) in self.layers[depth + depth_change]
                        ):
                            count += 1
                    # print("rar", depth, current, count)
                    if current in bugs:
                        if count == 1:
                            new_bugs.add(current)
                    else:
    ...
    def neighbours(self, pos):
        ...
        if pos in self.outer_transitions:
